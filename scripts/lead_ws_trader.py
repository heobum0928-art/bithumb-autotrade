        ...
        try:
            ...
            for coin in list(pos.keys()):
                ...
                if sl_hit or trail_hit or to_hit:
                    ...
                    try:
                        from bithumb import notify
                        notify.send(f"⚡ 리드WS 청산 {coin} {pnl:+.1f}% [{reason}] (lead{p['lead']:.1f})")
                    except Exception: pass
            ...
            if len(pos) < SLOTS:
                for coin in syms:
                    ...
                    if lead >= ENTER_LEAD and up_mom >= ENTER_LEAD and bh_mom < up_mom:
                        ...
                        log.info(f"[모의] 진입 {coin} @{bhp:,.4f} — lead{lead:.1f} (업비트{up_mom:+.1f}% 빗썸{bh_mom:+.1f}%) {tag}")
                        try:
                            from bithumb import notify
                            notify.send(f"⚡ 리드WS 진입 {coin} lead{lead:.1f} (업비트선행{up_mom:+.1f}%) {tag} [모의]")
                        except Exception: pass
