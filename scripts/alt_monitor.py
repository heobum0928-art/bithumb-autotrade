...
ALT_ENTRY_RATIO      = 0.015 # 소량 테스트 (0.05→0.015, ~15,000원)
        ...
        while True:
            ...
            for coin in candidates:
                try:
                    ...
                    if state == "idle" and rsi < 30:
                        _oversold_rsi_state[coin] = "watching"
                        _oversold_min_rsi[coin] = rsi
                        log.info(f"[OVERSOLD] {coin} RSI={rsi:.1f} < 30 → 반등 대기")
                        try:
                        ...
                        if rsi > 50:
                            ...
                            if cnt < 1:
                                log.info(
                                    ...
                                    f"({cnt}/1 확인 중, 다음 체크 대기)"
                                )
                            else:
                                # MACD/EMA 조건 제거 (데이터수집 목적, 거래량만 확인)
                                ...
                                if vol_ok:
                                    ...
                                    notify.send(
                                        f"🌊 [과매도 반등] {coin}\n"
                                        f"RSI={rsi:.1f} 거래량{vol_r:.1f}x → 진입 시도 중..."
                                    )
                                    _entry_ready.put({
                                        "coin": coin,
                                        "price": current,
