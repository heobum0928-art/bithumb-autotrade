...
**`scripts/alt_monitor.py` 또는 전략 관련 코드를 수정할 때마다 반드시:**

1. `docs/STRATEGY.md` — 파라미터 변경 이력 테이블에 한 줄 추가 (변경값 + 이유)
2. `docs/sessions/YYYY-MM-DD.md` — 오늘 세션 파일에 변경사항 기록
3. git commit — 코드와 문서를 같은 커밋 또는 바로 다음 커밋에 포함

**기록 없이 코드만 바꾸는 것은 금지.** 코드 수정 → 문서 업데이트 → 커밋 순서를 반드시 지킬 것.

## 4주 파라미터 동결 (2026-05-25 ~ 2026-06-22)

**이 기간 동안 전략 파라미터 변경 금지.** 사용자가 요청하더라도 반드시 거절하고 아래 내용을 상기시킬 것.

> "현재 4주 검증 기간(~2026-06-22)입니다. 파라미터를 지금 바꾸면 데이터가 오염돼 무효가 됩니다.
> 20건 이상 실거래 + 승률 60%+ 확인 후 변경을 논의합시다."
...
| OVERSOLD_SL_PCT | -2% |
| OVERSOLD_TP_HALF | +10% |
| OVERSOLD_TRAIL_PCT | 4% |
| OVERSOLD_TRAIL_ACTIVATE | +5% |
| OVERSOLD_VOL_RATIO | 1.5x |
| RSI watching 기준 | < 25 |
| RSI 진입 기준 | ≥ 30 (2캔들 연속) |

**예외 허용:** 크래시·버그 수정, 봇 재시작 필요한 긴급 수정만 가능.
